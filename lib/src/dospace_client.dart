...
class Client {
  ...
  Map<String, dynamic> signRequestDio(String httpMethod, Uri uri,
      {Digest? contentSha256, bool preSignedUrl = false, int expires = 86400, Map<String, dynamic>? headers}) {
    // Build canonical request
    // String httpMethod = request.options.method;
    // final uri = Uri.parse(request.options.baseUrl);
    ...
    // Map<String, String?> headers = new Map<String, String?>();
    if (!preSignedUrl) {
      _headers['x-amz-date'] = dateIso8601; // Set date in header
      if (contentSha256 != null) {
        _headers['x-amz-content-sha256'] =  hashedPayloadStr; // Set payload hash in header
      }
      headers?.keys.forEach((String name) =>
          (_headers[name.toLowerCase()] = headers[name]));
    }
    _headers['host'] = host; // Host is a builtin header
    List<String> headerNames = _headers.keys.toList()..sort();
    String canonicalHeaders =
        headerNames.map((s) => '${s}:${_trimAll(_headers[s]!)}' + '\n').join();

    String signedHeaders = headerNames.join(';');

    // Build canonical query string
    Map<String, String> _queryParameters = Map<String, String>();
    // ..addAll(request.options.queryParameters.map((key, value) => MapEntry(key, '$value')));
    if (preSignedUrl) {
      // Add query parameters
      _queryParameters['X-Amz-Algorithm'] = 'AWS4-HMAC-SHA256';
      _queryParameters['X-Amz-Credential'] = credential;
      _queryParameters['X-Amz-Date'] = dateIso8601;
      _queryParameters['X-Amz-Expires'] = expires.toString();
      if (contentSha256 != null) {
        _queryParameters['X-Amz-Content-Sha256'] = hashedPayloadStr;
      }
      _queryParameters['X-Amz-SignedHeaders'] = signedHeaders;
    }
    List<String> queryKeys = _queryParameters.keys.toList()..sort();
    String canonicalQueryString = queryKeys
        .map((s) => '${_uriEncode(s)}=${_uriEncode(_queryParameters[s]!)}')
        .join('&');
    ...
    if (preSignedUrl) {
      _queryParameters['X-Amz-Signature'] = '$signature';
